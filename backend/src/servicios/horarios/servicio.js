...
import {
  insertarClase, vincularAlumnoAClase, desvincularAlumnoDeClase, buscarClasePorId, buscarCruce, listarClases,
...
/**
 * NUEVO (2026-08-19): suma un alumno a una clase que YA existe (a
 * diferencia de crearClaseNueva, que crea la clase desde cero). Lo usa
 * el botón "+ agregar estudiante" y el arrastrar-soltar cuando la celda
 * destino ya tiene una clase del mismo programa.
 */
export async function agregarAlumnoAClase(claseId, alumnoId, contextoAuditoria) {
  const clase = await buscarClasePorId(claseId);
  if (!clase) throw new Error("La clase no existe.");
  if (!alumnoId) throw new Error("Falta indicar el alumno.");

  await vincularAlumnoAClase(claseId, alumnoId);

  await registrarAuditoria({
    usuarioId: contextoAuditoria?.usuarioId ?? null, accion: "agregar_alumno", modulo: "horarios",
    entidad: "clase", entidadId: claseId, resultado: "exito",
  });

  return { id: claseId, alumnos: await alumnosDeClase(claseId) };
}

/**
 * NUEVO (2026-08-19): quita un alumno de una clase. La clase sigue
 * existiendo aunque quede sin nadie — borrar la clase completa sería
 * una acción aparte, no pedida acá.
 */
export async function quitarAlumnoDeClase(claseId, alumnoId, contextoAuditoria) {
  const clase = await buscarClasePorId(claseId);
  if (!clase) throw new Error("La clase no existe.");

  await desvincularAlumnoDeClase(claseId, alumnoId);

  await registrarAuditoria({
    usuarioId: contextoAuditoria?.usuarioId ?? null, accion: "quitar_alumno", modulo: "horarios",
    entidad: "clase", entidadId: claseId, resultado: "exito",
  });
